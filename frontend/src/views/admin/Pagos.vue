...
const MODULE_NAME = 'Pago';

const fields = [
  { key: 'ID_Pago',     label: 'ID Pago',    type: 'number',  readOnly: true },
  { key: 'ID_Factura',  label: 'ID Factura', type: 'number',  required: true },
  { key: 'Fecha_Pago',  label: 'Fecha Pago', type: 'date',    required: true },
  { key: 'Monto_Pago',  label: 'Monto ($)',  type: 'decimal', required: true },
...
<template>
  <div class="crud-view">
    <div class="page-header">
      <h1 class="page-title text-primary">Módulo Pagos</h1>
      <p class="text-muted">Gestiona los pagos realizados por los clientes.</p>
    </div>
    <div style="display:flex; justify-content:flex-end; margin-bottom:2rem;">
      <button @click="openCreate" class="btn-primary" style="padding:0.75rem 2rem; display:flex; align-items:center; gap:0.5rem;">
        <span style="font-size:1.25rem;">+</span> Nuevo Pago
      </button>
    </div>
    <div class="glass-panel" style="padding:0; overflow:hidden;">
      ...
      <div v-else style="padding:4rem; text-align:center;"><h3 class="text-muted">Sin pagos registrados</h3><button @click="openCreate" class="btn-primary" style="margin-top:2rem;">Registrar primer pago</button></div>
    </div>
