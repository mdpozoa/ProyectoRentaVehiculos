...
<template>
  <div class="crud-view">
    <div class="page-header">
      <h1 class="page-title text-primary">Módulo Usuarios</h1>
      <p class="text-muted">Gestiona los usuarios y roles del sistema.</p>
    </div>
    <div style="display:flex; justify-content:flex-end; margin-bottom:2rem;">
      <button @click="openCreate" class="btn-primary" style="padding:0.75rem 2rem; display:flex; align-items:center; gap:0.5rem;">
        <span style="font-size:1.25rem;">+</span> Nuevo Usuario
      </button>
    </div>
          ...
          <tbody>
            <tr v-for="item in items" :key="item[PK]">
              <td v-for="f in fields" :key="f.key">
                <span v-if="f.key === 'Pass_Usuario'">••••••</span>
