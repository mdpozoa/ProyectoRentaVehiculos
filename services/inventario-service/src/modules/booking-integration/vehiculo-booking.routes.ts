...
export function createVehiculoBookingRouter(repo: VehiculoRepository): Router {
  ...
  // GET /api/v1/mateodavid/vehiculos/booking/:id/disponibilidad  — must be before /:id
  ...
  // PATCH /api/v1/mateodavid/vehiculos/booking/:id/reservar (Internal Webhook for operaciones-service)
  router.patch('/:id/reservar', async (req, res, next) => {
    try {
      ...
      const result = await repo.update(vehiculo.id, { status: 'RESERVADO' });
      res.json({ success: true, data: toBookingDto(result) });
    } catch (err) { next(err); }
  });
