...
app.get('/api/v1/mateodavid/admin/dashboard', authenticate, requireAdmin, async (_req, res, next) => {
  try {
...
app.get('/api/v1/mateodavid/historial', authenticate, requireAdmin, async (req, res, next) => {
  try {
...
app.get('/api/v1/mateodavid/outbox-events', authenticate, requireAdmin, async (req, res, next) => {
  try {
    const page  = Number(req.query.page)  || 1;
    const limit = Number(req.query.limit) || 50;
    const [total, events] = await Promise.all([
...
app.use('/api/v1/mateodavid/reservas/booking',    createReservaBookingRouter(reservaRepository));
app.use('/api/v1/mateodavid/alquileres/booking',  createAlquilerBookingRouter(alquilerRepository));
app.use('/api/v1/mateodavid/devoluciones/booking', createDevolucionBookingRouter(alquilerRepository));
...
app.post(
  '/api/v1/mateodavid/devoluciones',
  authenticate,
  requireAdmin,
