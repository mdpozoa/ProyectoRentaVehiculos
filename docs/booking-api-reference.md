...
Los servicios de backend son internos (no tienen acceso público directo). Todos los endpoints tienen el prefijo `/api/v1/mateodavid` y las rutas de integración llevan adicionalmente `/booking`.

Ejemplo completo:
```
GET https://nginx-frontend.ambitioushill-8cbf622c.eastus2.azurecontainerapps.io/api/v1/mateodavid/vehiculos/booking
```
